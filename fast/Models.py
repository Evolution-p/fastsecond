...
class NestedUNet_att(nn.Module):
    """
    Implementation of this paper:
    https://arxiv.org/pdf/1807.10165.pdf
    """
    def __init__(self, in_ch=3, out_ch=1):
        super(NestedUNet_att, self).__init__()
        ...
        self.Up0 = up_conv(filters[1], filters[0])
        self.Up1 = up_conv(filters[2], filters[1])
        self.Up2 = up_conv(filters[3], filters[2])
        self.Up3 = up_conv(filters[4], filters[3])

        self.Att0 = Attention_block(F_g=filters[0], F_l=filters[0], F_int=filters[0]//2)
        self.Att1 = Attention_block(F_g=filters[1], F_l=filters[1], F_int=filters[1]//2)
        self.Att2 = Attention_block(F_g=filters[2], F_l=filters[2], F_int=filters[2]//2)
        self.Att3 = Attention_block(F_g=filters[3], F_l=filters[3], F_int=filters[3]//2)
        ...
        self.conv0_1 = conv_block_nested(filters[1], filters[0], filters[0])
        self.conv1_1 = conv_block_nested(filters[2], filters[1], filters[1])
        self.conv2_1 = conv_block_nested(filters[3], filters[2], filters[2])
        self.conv3_1 = conv_block_nested(filters[4], filters[3], filters[3])
    ...
    def forward(self, x):
        ...
        x0_0_att = self.Att0(g=self.Up0(x1_0),x=x0_0)
        x0_1 = self.conv0_1(torch.cat([x0_0_att, self.Up0(x1_0)], 1))


        x2_0 = self.conv2_0(self.pool(x1_0))
        x1_0_att = self.Att1(g=self.Up1(x2_0), x=x1_0)
        x1_1 = self.conv1_1(torch.cat([x1_0_att, self.Up1(x2_0)], 1))
        x0_2 = self.conv0_2(torch.cat([x0_0, x0_1, self.Up(x1_1)], 1))

        x3_0 = self.conv3_0(self.pool(x2_0))
        x2_0_att = self.Att2(g=self.Up2(x3_0), x=x2_0)
        x2_1 = self.conv2_1(torch.cat([x2_0_att, self.Up2(x3_0)], 1))
        x1_2 = self.conv1_2(torch.cat([x1_0, x1_1, self.Up(x2_1)], 1))
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, x0_2, self.Up(x1_2)], 1))

        x4_0 = self.conv4_0(self.pool(x3_0))
        x3_0_att = self.Att3(g=self.Up3(x4_0), x=x3_0)
        x3_1 = self.conv3_1(torch.cat([x3_0_att, self.Up3(x4_0)], 1))
...
class NestedUNet_mult_att(nn.Module):
    """
    Implementation of this paper:
    https://arxiv.org/pdf/1807.10165.pdf
    """
    def __init__(self, in_ch=3, out_ch=1):
        super(NestedUNet_mult_att, self).__init__()
    ...
    def forward(self, x):
        ...
        x1_0 = self.conv1_0(self.pool(x0_0))
        #x0_0_att = self.Att0(g=self.Up0(x1_0),x=x0_0)
        x0_1 = self.conv0_1(torch.cat([self.Att0(g=self.Up0(x1_0),x=x0_0), self.Up0(x1_0)], 1))


        x2_0 = self.conv2_0(self.pool(x1_0))
        x1_0_att = self.Att1(g=self.Up1(x2_0), x=x1_0)
        x1_1 = self.conv1_1(torch.cat([x1_0_att, self.Up1(x2_0)], 1))
        #x0_1_att = self.Att0(g=self.Up0(x1_1), x=x0_1)
        x0_2 = self.conv0_2(torch.cat([x0_0, self.Att0(g=self.Up0(x1_1), x=x0_1), self.Up(x1_1)], 1))

        x3_0 = self.conv3_0(self.pool(x2_0))
        x2_0_att = self.Att2(g=self.Up2(x3_0), x=x2_0)
        x2_1 = self.conv2_1(torch.cat([x2_0_att, self.Up2(x3_0)], 1))
        #x1_1_att = self.Att1(g=self.Up1(x2_1), x=x1_1)
        x1_2 = self.conv1_2(torch.cat([x1_0, self.Att1(g=self.Up1(x2_1), x=x1_1), self.Up(x2_1)], 1))
        #x0_2_att = self.Att0(g=self.Up0(x1_2), x=x0_2)
        x0_3 = self.conv0_3(torch.cat([x0_0, x0_1, self.Att0(g=self.Up0(x1_2), x=x0_2), self.Up(x1_2)], 1))

        x4_0 = self.conv4_0(self.pool(x3_0))
        x3_0_att = self.Att3(g=self.Up3(x4_0), x=x3_0)
        x3_1 = self.conv3_1(torch.cat([x3_0_att, self.Up3(x4_0)], 1))
        #x2_1_att = self.Att2(g=self.Up2(x3_1), x=x2_1)
        x2_2 = self.conv2_2(torch.cat([x2_0, self.Att2(g=self.Up2(x3_1), x=x2_1), self.Up(x3_1)], 1))
        #x1_2_att = self.Att1(g=self.Up1(x2_2), x=x1_2)
        x1_3 = self.conv1_3(torch.cat([x1_0, x1_1, self.Att1(g=self.Up1(x2_2), x=x1_2), self.Up(x2_2)], 1))
        #x0_3_att = self.Att0(g=self.Up0(x1_3), x=x0_3)
        x0_4 = self.conv0_4(torch.cat([x0_0, x0_1, x0_2, self.Att0(g=self.Up0(x1_3), x=x0_3), self.Up(x1_3)], 1))

        output = self.final(x0_4)
        return output

class NestedUNet_R2Att(nn.Module):
    """
    Implementation of this paper:
    https://arxiv.org/pdf/1807.10165.pdf
    """
