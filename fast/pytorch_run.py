...
#torchsummary.summary(model_test, input_size=(3, 480, 320))
#######################################################
...
t_data = './data/new_img5/'
l_data = './data/new_gt5/'
test_image = './data/test/004.png'
test_label = './data/test/004.png'
test_folderP = './data/test/*'
...
New_folder = './model3_NestedUNet_att'
...
read_pred = './model3_NestedUNet_att/pred'
...
read_model_path = './model3_NestedUNet_att/NestedUNet_att_' + str(epoch) + '_' + str(batch_size)
...
for i in range(epoch):
   ...
   #pred_tb = threshold_predictions_v(pred_tb)
    ...
    x1 = plt.imsave(
        './model3_NestedUNet_att/pred/img_iteration_' + str(n_iter) + '_epoch_'
    ...
    if (i+1) % 1 == 0:
        ...
        #writer1.add_scalar('Accuracy', accuracy, n_iter)
    ...
    if valid_loss <= valid_loss_min and epoch_valid >= i: # and i_valid <= 2:
        ...
        torch.save(model_test.state_dict(),'./model3_NestedUNet_att/NestedUNet_att_' +
...
test1 =model_test.load_state_dict(torch.load('./model3_NestedUNet_att/NestedUNet_att_' +
                   str(epoch) + '_' + str(batch_size)+ '/Unet_epoch_' + str(epoch)
                   + '_batchsize_' + str(batch_size) + '.pth'))
...
model_test.load_state_dict(torch.load('./model3_NestedUNet_att/NestedUNet_att_' +
                   str(epoch) + '_' + str(batch_size)+ '/Unet_epoch_' + str(epoch)
                   + '_batchsize_' + str(batch_size) + '.pth'))
...
read_test_folder112 = './model3_NestedUNet_att/gen_images'
...
read_test_folder_P_Thres = './model3_NestedUNet_att/pred_threshold'
...
read_test_folder_L_Thres = './model3_NestedUNet_att/label_threshold'
...
#    pred = threshold_predictions_p(pred) #Value kept 0.01 as max is 1 and noise is very small.
    ...
    x1 = plt.imsave('./model3_NestedUNet_att/gen_images/im_epoch_' + str(epoch) + 'int_' + str(i)
...
read_test_folderP = glob.glob('./model3_NestedUNet_att/gen_images/*')
...
for i in range(len(read_test_folderP)):
    ...
    #save the images
    x1 = plt.imsave('./model3_NestedUNet_att/pred_threshold/im_epoch_' + str(epoch) + 'int_' + str(i)
   ...
   # s2 =threshold_predictions_v(s2)
    ...
    #save the Images
    y1 = plt.imsave('./model3_NestedUNet_att/label_threshold/im_epoch_' + str(epoch) + 'int_' + str(i)
